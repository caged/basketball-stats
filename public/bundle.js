  ...
  step: function () {
    ...
    for (var i = 0; i < N; i++) {
      for (var d = 0; d < this.dim; d++) {
        ...
        // compute gain update
        var newgain = sign$2(gid) === sign$2(sid) ? gainid * 0.8 : gainid + 0.2;
...
function render(data) {
  ...
  const timescale = linear$2().domain([0, 20, 50, 100, 200, 6000]).range([60, 30, 20, 10, 0]);
  ...
  const x = linear$2().range([0, width]);
  ...
  function run() {
    ...
    select('.js-pause').on('click', function () {
  ...
  function slider(el, name, min, max, val) {
    const sect = el.append('div').attr('class', 'slider');

    sect.append('span').attr('class', `slider-label slider-label-${name}`).text(name);

    const value = sect.append('span').attr('class', `slider-value slider-value-${name}`).text(val);

    const slider = sect.append('input').attr('class', 'option').attr('type', 'range').attr('min', min).attr('max', max).attr('value', val).attr('name', name.toLowerCase()).on('change', () => {
      updateOptions();
      run();
...
csv$1('players-with-position.csv').get(render);
